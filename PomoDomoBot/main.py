...
TOKEN = 'ODIyNTc2ODE0ODIwMDMyNTEy.G5VeMa.wBBjoEhyKV4a979qSDVJYwmH5VpSHi4FHaumhU'
BOT_PREFIX = '>'
...
async def timer(ctx, *args):
    ...
    if isinstance(int(args[0]), int) and isinstance(int(args[1]), int) and isinstance(int(args[2]), int):
        ...
        while True:
            t = time.time()
            await ctx.send(f"{smallBreaks}/4 work cycles in progress. Next break at {time.strftime('%I:%M:%S %p', time.localtime(t + workTime))}. Type !stop to stop the timer.")
            if smallBreaks != 4:
                smallBreaks += 1
                time.sleep(workTime)
